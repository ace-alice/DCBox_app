...
class JSBridge {
  ...
  /// 监听jsbridge消息
  void onMessageReceived(Object messageObject) {
    String decodeString = '';
    if (messageObject is String) {
      decodeString = Uri.decodeFull(messageObject);
    }
    if (messageObject is List) {
      if (messageObject.isNotEmpty) {
        decodeString = Uri.decodeFull(messageObject[0]);
      }
    }
    if (decodeString.isEmpty) return;
    Map<String, dynamic> jsonData = jsonDecode(decodeString);
